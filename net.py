...
# Render'ın istediği "health check" endpoint
@app.route("/")
def home():
    return "✅ Bot çalışıyor!"

if __name__ == "__main__":
    print("✅ Bot Render üzerinde çalışıyor (4h ve 1d taraması yapacak).")
    bot.send_message(chat_id=TG_CHAT_ID, text="✅ Bot Render üzerinde başlatıldı!")
    scheduler = BackgroundScheduler()
    scheduler.add_job(job, 'interval', minutes=60)  # 1 saatte bir
    scheduler.start()
    app.run(host="0.0.0.0", port=10000)  # Render için gerekli
